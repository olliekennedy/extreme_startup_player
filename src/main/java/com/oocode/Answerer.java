package com.oocode;

import java.math.BigInteger;

public class Answerer {
    public String answerFor(String question) {
        String[] listOfWords = question.substring(0, question.length() - 1).split(" ");

        if (question.matches("^What is your name\\?$")) {
            return "Jack will win this for us";
        }

        if (question.matches("^What is \\d+ plus \\d+\\?$")) {
            return sumOf(listOfWords[2], listOfWords[4]);
        }

        if (question.matches("^What is \\d+ multiplied by \\d+\\?$")) {
            return Integer.toString(Integer.parseInt(listOfWords[2]) * Integer.parseInt(listOfWords[5]));
        }

        if (question.matches("^What is \\d+ minus \\d+\\?$")) {
            return Integer.toString(Integer.parseInt(listOfWords[2]) - Integer.parseInt(listOfWords[4]));
        }

        if (question.matches("^What is \\d+ multiplied by \\d+ plus \\d+\\?$")) {
            return Integer.toString(((Integer.parseInt(listOfWords[2]) * Integer.parseInt(listOfWords[5])) + Integer.parseInt(listOfWords[7])));
        }

        if (question.matches("^What is \\d+ plus \\d+ multiplied by \\d+\\?$")) {
            return Integer.toString((Integer.parseInt(listOfWords[2]) + (Integer.parseInt(listOfWords[4]) * Integer.parseInt(listOfWords[7]))));
        }

        if (question.matches("^What is \\d+ plus \\d+ plus \\d+\\?$")) {
            return sumOf(listOfWords[2], listOfWords[4], listOfWords[6]);
        }

        if (question.matches("^What is \\d+ to the power of \\d+\\?$")) {
            return new BigInteger(listOfWords[2]).pow(Integer.parseInt(listOfWords[7])).toString();
        }

        return "foo";
    }

    private String sumOf(String... numbers) {
        int accumulator = 0;
        for (String number : numbers) {
            accumulator += Integer.parseInt(number);
        }
        return Integer.toString(accumulator);
    }
}
